...
class App:
    ...
    def __init__(self):
        pyxel.init(self.SCREEN_W, self.SCREEN_H, title="Rocket's Revenge", fps=30)
        ...
        self.map_manager = MapManager(self, ROOM_DATA, self.ROOM_TILES_W, self.ROOM_TILES_H)
    ...
    def attempt_move(self, next_x, next_y, dx, dy):
        ...
        if (direction == "R" and next_x == self.ROOM_TILES_W) or \
           (direction == "L" and next_x == -1):
            is_transition = True

        # Check for U/D transitions
        if (direction == "D" and next_y == self.ROOM_TILES_H) or \
        ...
        if is_transition and direction in current_room["neighbors"]:
            ...
            if direction == "R":
                ...
                target_x = self.ROOM_TILES_W - 2 # Start on the right side of the new room
            elif direction == "D":
                target_y = 1 # Start on the top side of the new room
            elif direction == "U":
                target_y = self.ROOM_TILES_H - 2 # Start on the bottom side of the new room
        ...
        if 0 <= next_x < self.ROOM_TILES_W and 0 <= next_y < self.ROOM_TILES_H:
    ...
    def draw(self):
        ...
        if self.map_manager.transition_state == "SCROLLING":
            ...
            player_screen_x = (self.player.x_tile * self.TILE_SIZE) - self.map_manager.scroll_offset_x
            player_screen_y = (self.player.y_tile * self.TILE_SIZE) - self.map_manager.scroll_offset_y
        else:
            # Not scrolling: Player is drawn at their current tile position in the fixed room
            player_screen_x = self.player.x_tile * self.TILE_SIZE
            player_screen_y = self.player.y_tile * self.TILE_SIZE
        ...
        if self.game_message:
            pyxel.text(5, self.SCREEN_H - 14, self.game_message, pyxel.COLOR_ORANGE)

        pyxel.text(5, self.SCREEN_H - 7, "Use Arrow Keys. (Q) to quit.", pyxel.COLOR_GRAY)
